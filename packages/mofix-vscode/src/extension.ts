 ...
 */
function loadMoFixConfig(root: string): MoFixConfig | null {
  try {
    ...
    return JSON.parse(raw) as MoFixConfig;
  } catch (err) {
 ...
 */
function buildReviewUrl(cfg: MoFixConfig | null): string {
...
function runBuild(
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on('close', (code, signal) => {
      channel.appendLine('');
      channel.appendLine(
        `[MoFix] Build process exited with code=${code}, signal=${signal ?? 'null'}`
      );
      resolve({ code, signal });
    });
  });
  ...
  try {
    ...
    if (code === 0) {
      ...
      const choice = await vscode.window.showInformationMessage(
        `✅ Build succeeded for "${projectName}". Would you like to write a review for MoFix?`,
        'Write a review',
        'Later'
      );
      ...
      vscode.window.showErrorMessage(
        `MoFix: build failed for "${projectName}". Open the "MoFix" output panel for details.`
      );
    }
  } catch (err) {
    console.error('[MoFix] verifyBuild failed', err);
    vscode.window.showErrorMessage(
      'MoFix: Failed to run build. See "MoFix" output for details.'
    );
  }
}
...
export function activate(context: vscode.ExtensionContext) {
  ...
  const openReview = vscode.commands.registerCommand(
    'mofix.openReview',
    openReviewCommand
  );
